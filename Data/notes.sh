...
path="ntfsv6"
if [ -f $path ] ;then
    echo "exists and is a regular file. $path."
else
    echo "Not exists OR is not a regular file. $path."
fi
#* ------------------------------ > test >
...
echo $1 $2 $3 $4
# * ------------------------------ >>
# getopts >> تستخدم علشان نعمل ال arg الي بيكون جنب الاسكربت 
# ./bash.sh -v "test" -h >> بتكون كدا مثلا -v بتاخد قيمة و ال -h مش بتاخذ قيمة
# x: >> دي كدا معنها ان ال -x لازم تاخد قيمه  ( -x "Test" )
# y >> دي كدا ال -y مش لازم تاخد قيمه  ( -y )
# args >> هي المدخلات الي هيدخلها المستخدم
# ${OPTARG} >> هي القيمه الي حطينها جنب ال arg  ( ./bash.sh -x "From X" -y -z "From Z" )
# ${OPTARG} >> From X , From Z
# shift $((OPTIND -1)) >> دي كدا معنها بعد كل العناصر الي دخلنها ممكن نكتب حاجات تانيه
# اكنك بالظبط خليات العناصر الي في الاخر دي هي اول عناصر لانك عملتلهم إزاحة
# ./bash.sh -x "From X" -y -z "From Z" "From OPT1" "From OPT2"
# echo "arg \$1 is : $1" >> From OPT1
# echo "arg \$2 is : $2" >> From OPT2
usage(){
    echo "Usage : $0 [-x <options>] [-y] [-z <options>]"
    echo "Invalid Arguments"
}
while getopts "x:yz:" args;do
    case $args in 
    x) echo "Hello From x arg Value : ${OPTARG}";;
    y) echo "Hello From y arg No Value";;
    z) echo "Hello From z arg Value : ${OPTARG}";;
    *) usage
    esac
done
shift $((OPTIND -1))
...
echo "arg \$2 is : $2"
#? ------------------------------------- 48 -------------------------------------
# Video 48 => https://www.youtube.com/watch?v=fpFYcfARdiQ&list=PLBdyyeW_Z41DykncH9zzMk8T7Rm5UlZXd&index=48

#                          >>>>>>>>> Declare <<<<<<<<<<
#? ------------------------------------- 48 -------------------------------------
# . bash.sh >> علشان نستخدم ال decalre لازم نشغل الاسكربت بهذا الشكل 
# declare -f decl >> استخدمنا -f علشان نقدر نعرض محتوي ال function الي جوه الاسكربت
# echo $testval >> وقدرنا نعرض محتوي المتغير الي جوه ملف الاسكربت بشكل طبيعي
function decl(){
    echo "From Decl Function.."
}
testval=55
decl
# * ------------------------------ >> OUT OF COURSE
# -f: عرض كود الدالة كاملاً.
my_func() { echo "Hello"; }
declare -f my_func
# * ------------------------------ >> OUT OF COURSE
# -F: عرض اسم الدالة فقط.
declare -F my_func
# * ------------------------------ >> OUT OF COURSE
# -g: تعريف متغير عام (Global) من داخل دالة.
test_func() {
  declare -g x=10
}
test_func
echo $x # سيطبع 10
# * ------------------------------ >> OUT OF COURSE
# -I: وراثة خصائص متغير سابق بنفس الاسم.
x=100
func() {
  declare -I x
  echo $x # سيطبع 100 (ورث القيمة من الخارج)
}
func
# * ------------------------------ >> OUT OF COURSE
# -p: عرض خصائص وقيمة المتغير الحالية.
name="Ahmed"
declare -p name
# الناتج: declare -- name="Ahmed"
# * ------------------------------ >> OUT OF COURSE
# -a: مصفوفة عادية (بأرقام: 0, 1, 2).
declare -a list=(0 1 2)
echo ${list[1]} # سيطبع 1
# * ------------------------------ >> OUT OF COURSE
# -A: مصفوفة مترابطة (بكلمات: [key]=value).
declare -A user=([id]=1 [name]="Ali")
echo ${user[name]} # سيطبع Ali
# * ------------------------------ >> OUT OF COURSE
# -i: متغير حسابي (للقيم العددية فقط).
declare -i result
result=5*5
echo $result # سيطبع 25 (وليس النص 5*5)
# * ------------------------------ >> OUT OF COURSE
# -l: تحويل النص تلقائياً لحروف صغيرة.
declare -l text="HELLO"
echo $text # سيطبع hello
# * ------------------------------ >> OUT OF COURSE
# -u: تحويل النص تلقائياً لحروف كبيرة.
declare -u text="bash"
echo $text # سيطبع BASH
# * ------------------------------ >> OUT OF COURSE
# -n: جعل المتغير "مرجع" أو "مختصر" لاسم متغير آخر.
real_var="Original"
declare -n ref=real_var
ref="Changed"
echo $real_var # سيطبع Changed
# * ------------------------------ >> OUT OF COURSE
# -r: متغير للقراءة فقط (لا يمكن تعديله).
declare -r pi=3.14
pi=4 # سيظهر خطأ: readonly variable
echo $pi
# * ------------------------------ >> OUT OF COURSE
# -x: تصدير المتغير كمتغير بيئة (Environment Variable).
declare -x MY_ENV="production"
# * ------------------------------ >> OUT OF COURSE
# -t: تتبع المتغير لأغراض التصحيح (Debug).
#? ------------------------------------- 49 -------------------------------------
# Video 49 => https://www.youtube.com/watch?v=NRP5KzivAK0&list=PLBdyyeW_Z41DykncH9zzMk8T7Rm5UlZXd&index=49
#                          >>>>>>>>> Lab <<<<<<<<<<
#? ------------------------------------- 49 -------------------------------------
# عملنا اسكربت بتشوف هل الملفين بيتساوي ام لا من خلال استخدام ال md5sum
test1=`md5sum $1`
test2=`md5sum $2`
arr1=($test1)
arr2=($test2)
